...
hammerSlide = cv2.imread(os.path.join(resourceDirectory,"dauntless balance pin.png"))
uai = bestOf.userArtificialInput()
resolution = bestOf.getResolution()
hammerPinBox = 400, 166,440, 206
threshold = 0.9
...
def mapWindow(secondaryMonitorSize=(2560,1440)):
    ...
    while not closeIt:
        #capture map image
        imz = ImageGrab.grab(bbox=hammerPinBox)
        rgb = np.array(imz)
...
def mane():
    ...
    #click hammerSlide button and ban techies at the start of the game.
    while closeIt == False:
        #check for hammerSlide button
        acceptClicked = uai.clickImage(hammerSlide, threshold)
        if acceptClicked:
